...
RESOURCE_MAPPING = {
    ...
    "slots"            : ("slots",),
    "s_vmem"           : ("s_vmem", "mem_mb",  "soft_memory", "soft_virtual_memory"),
...
def format_job_properties(string):
    ...
    if job_properties['type'] == 'group':
        return string.format(rulename=job_properties['groupid'], jobid=job_properties['jobid'])
...
if "threads" in job_properties:
    qsub_settings["options"]["pe"] = qsub_settings["options"]["pe"] + " " + str(job_properties["threads"])
else:
    qsub_settings["options"]["pe"] = qsub_settings["options"]["pe"] + " 1"
